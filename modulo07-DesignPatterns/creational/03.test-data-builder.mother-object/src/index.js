...
function productValidation(product) {
    ...
    if(!(product.price >= 1 && product.price <= 1000)) {
        errors.push(`price: invalid value, current [${product.price}] expected should be from zero to a thousand`)
    }

    if(! (['electronic', 'organic'].includes(product.category))) {
        errors.push(`category: invalid value, current [${product.category}] should be electronic or organic`)
    }  
