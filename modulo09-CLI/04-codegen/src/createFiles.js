...
export async function createFiles({ mainPath, defaultMainFolder, layers, componentName }) {
    ...
    for (const layer of layers) {
        ...
        if (!chosenTemplate) {
            return { error: 'the closen doesnt have a template' }
        }
        ...
        const { fileName: nameFile, template: txtFile } = template(componentName, ...dependencies)
        const fileName = `${targetFolder}/${Util.lowerCaseFirstLetter(nameFile)}.js`
